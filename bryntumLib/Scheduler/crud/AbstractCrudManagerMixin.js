    ...
    return class AbstractCrudManagerMixin extends (Target || Base).mixin(...mixins) {
        ...
        static get defaultConfig() {
            return {
                 ...
                 * Each store is represented by a _store descriptor_, an object having the following structure:
                 ...
                 * @property {String} [crudStores.phantomIdField] Set this if store model has a predefined field to
                 * keep phantom record identifier.
                 * @property {String} [crudStores.idField] id field name, if it's not specified then class will try to
                 * get it from a store model.
                 * @category CRUD
                 */
                 ...
                 * @property {String} stores.storeId Unique store identifier. Under this name the store related
                 * requests/responses will be sent.
                 * @property {Core.data.Store} stores.store The store itself.
                 * @property {String} [stores.phantomIdField] Set this if the store model has a predefined field to keep
                 * phantom record identifier.
                 * @property {String} [stores.idField] id field name, if it's not specified then class will try to get
                 * it from a store model.
                 ...
                 * @property {String} [syncApplySequence.phantomIdField] Set this if store model has a predefined field
                 * to keep phantom record identifier.
                 * @property {String} [syncApplySequence.idField] id field name, if it's not specified then class will
                 * try to get it from a store model.
                 * @category CRUD
                 */

                /**
                 * An array of store identifiers sets an alternative sync responses apply order.
                 * By default the order in which sync responses are applied to the stores is the same as they registered
                 * in. But in case of some tricky dependencies between stores this order can be changed:
                 *
                 *```javascript
                 * class MyCrudManager extends CrudManager {
        ...
        prepareAdded(list, storeInfo) {
            ...
            return list.filter(record => record.isValid).map(record => {
                ...
                if (this.resetIdsBeforeSync) {
                    delete ObjectHelper.deletePath(data, cls.getFieldDataSource(cls.idField));
                }
        ...
        applyChangesToStore(storeDesc, storeResponse, storePack) {
            ...
            return store.applyChangeset({ removed, added, updated }, null, phantomIdField, remote, true);
        }
        ...
        applySyncResponse(response, request) {
            ...
            const stmDisabled = me.project?.stm.disabled;

            if (me.project && me.ignoreRemoteChangesInSTM) {
                me.project.stm.disabled = true;
            }
            ...
            if (me.project) {
                me.project.stm.disabled = stmDisabled;
            }
