...
export default base => class SchedulerExporterMixin extends base {
    ...
    renderDependencies() {
        const
            me                      = this,
            { client, eventsBoxes } = me.exportMeta,
            { dependencies }        = client,
        ...
        dependencies.forEach(dependency => {
            if ((!eventsInView.includes(String(dependency.from)) &&
                !eventsInView.includes(String(dependency.to))) ||
                !dependencyFeature.isDependencyVisible(dependency)) {
                return;
            }

            const
                fromBox = eventsBoxes.get(String(dependency.from)),
        ...
        DomHelper.removeEachSelector(targetElement, '.b-sch-dependency');
