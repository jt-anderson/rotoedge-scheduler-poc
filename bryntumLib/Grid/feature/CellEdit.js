...
export default class CellEdit extends Delayable(InstancePlugin) {
    ...
    static get pluginConfig() {
        return {
            assign : ['startEditing'],
            before : ['onElementKeyDown', 'onElementMouseDown', 'onElementPointerUp'],
     ...
     * @fires startCellEdit
     * @returns {Boolean} Returns `true` if editing started
     * @category Editing
     * @on-owner
     */
    startEditing(cellContext = {}) {
        ...
        if (!me.disabled) {
            ...
            if (grid.store.isTree && grid.features.tree) {
                const record = cellContext.id ?? cellContext.record ?? grid.store.getAt(cellContext.row);
     ...
     * @category Editing
     * @returns {Boolean} `false` if the edit could not be finished due to the value being invalid or the
    ...
    onCellClick({ source : grid, cellSelector, target, event, column }) {
        ...
        if (target.closest('.b-tree-expander')) {
            ...
            me.onTriggerEditEvent({ cellSelector, target });
        }
    ...
    async onElementKeyDown(event) {
        ...
        if (!me.editorContext) {
            const
                ...
                autoEdit         = me.autoEdit && !ctrlKey && (key.length === 1 || backspace);
        ...
        if (await me.finishEditing()) {
            ...
            if (!me.isEditing) {
                ...
                // Edit next
                    ...
                    if (!me.isDestroyed && grid.internalNextPrevRow(true, true, event) && me.editNextOnEnterPress) {
                        me.startEditing(grid.focusedCell);
                    }
                }
            }
        ...
        if (focusedCell) {
            ...
            if (!cellInfo && isForward && me.addNewAtEnd) {
                ...
                if (currentEditableFinalizationResult === true) {
                    ...
                    cellInfo = !me.isDestroyed && me.getAdjacentEditableCell(focusedCell, isForward);
                }
            }
    ...
    onElementMouseDown(event) {
        if (this.editorContext && this.grid.isScrollbarOrRowBorderOrSplitterClick(event)) {
            this.cancelEditing();
        }
    }
