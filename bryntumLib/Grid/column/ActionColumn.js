...
export default class ActionColumn extends Column {
    ...
    static get fields() {
        return [
             ...
             * @property {Function|String|Object} actions.tooltip Tooltip text, or a config object which can
             ...
             * @property {Function|String} actions.renderer A render function, or the name of a function in the
             * Grid's ownership tree used to define the action element. Expected to return a HTML string or a DOM config object.
    ...
    updateAutoWidth() {
        ...
        if (groupActions.length > actions.length) {
            actions = column.actions = groupActions;
        }

        const actionsHtml = DomHelper.createElement(me.internalRenderer({ column, callExternalRenderer : false })).outerHTML;

        me.width = DomHelper.measureText(actionsHtml, me.element, true, me.element.parentElement);
        column.actions = oldActions;
    }
}
