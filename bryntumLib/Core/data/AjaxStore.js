...
export default class AjaxStore extends Store {
    ...
    performFilter(silent) {
        ...
        if (me.remoteFilter && !me.isRemoteDataLoading) {
            // Flag store data loading state to not get into loop when data is sorted internally after request
            me.isRemoteDataLoading = true;
     ...
     * Should not be used in application directly.
    ...
    internalLoad(params, eventName, successFn) {
        ...
        return url ? new Promise((resolve, reject) => {
            ...
            if (me.isPaged) {
                if (!((me.pageParamName in allParams) || (me.pageStartParamName in allParams))) {
                    const
