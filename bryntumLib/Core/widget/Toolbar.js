...
export default class Toolbar extends Container.mixin(Toolable) {
    ...
    syncOverflowVisibility() {
        ...
        const
            ...
            // RTL overflows when left edge is to the left of contentElement, left < 0
            overflowing = rtl
                ? Math.floor(itemRect.left) < 0
                : Math.floor(itemRect[edgeProp]) > availableSpace;

        if (visibleItems && overflowing) {
            if (menuOverflow) {
                ...
                while (overflowIndex-- > 0) {
                    ...
                    if (rtl) {
                        if (Math.floor(itemRect.left) > 0) {
                            break;
                        }
                    }
                    else if (Math.floor(itemRect[edgeProp]) <= availableSpace) {
                        break;
                    }
                }
    ...
    onLeafItemConfigChange(origResult, { name, value }) {
        ...
        if (!overflow || toolbar?.inSyncOverflowVisibility || (name === 'hidden' && value === me._toolbarOverflow)) {
            return;
        }

        // If the changed item has a clone in the overflow menu and the config is not one
        // of the unshared ones, sync the clone
        if (_toolbarOverflowClone && !nonSyncedConfigs[name]) {
        ...
        if (toolbar?.isPainted && me.ref !== 'overflowMenuButton') {
            if (!(me.isTextField && name === 'value' && (me.containsFocus || _toolbarOverflowClone?.containsFocus))) {
